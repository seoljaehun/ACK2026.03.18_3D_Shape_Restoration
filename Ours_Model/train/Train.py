...
def train_one_epoch(model, dataloader, optimizer, criterion, device, epoch):
    ...
    Args:
        model: 학습할 네트워크(UNet 기반 3D 복원 모델)
    ...
    for batch in loop:
        ...
        occ_target = batch['occupancies'].to(device).float()    # (B, N)
        scale = batch['scale'].to(device)        # (B, 1)
        loc = batch['loc'].to(device)            # (B, 3)
        camera_k = batch['inputs_k'].to(device)  # (B, 3, 3)
        camera_rt = batch['inputs_rt'].to(device)# (B, 3, 4) 또는 (B, 4, 4)
        
        # Gradient 초기화
        optimizer.zero_grad()
        
        # 모델 Forward 연산
        preds = model(images, points, scale, loc, camera_k, camera_rt)   # (B, N)
...
def validate_one_epoch(model, dataloader, criterion, device, epoch):
    ...
    with torch.no_grad():
        ...
        for batch in loop:
            ...
            # 모델 Forward 연산
            preds = model(images, points, scale, loc, camera_k, camera_rt)
