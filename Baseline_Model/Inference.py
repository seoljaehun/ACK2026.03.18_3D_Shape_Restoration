...
#=======================
class MeshGenerator:
    def __init__(self, model, device, resolution=128, threshold=0.2):
        """
            3D 좌표 그리드 생성 및 초기화
        Args:
    ...
    def generate_mesh(self, img):
        ...
        with torch.no_grad():
            ...
            c = self.model.encoder(img_tensor)
            ...
            for i in range(0, num_points, POINTS_BATCH_SIZE):
                ...
                # 모델에게 예측 요청 후 결과 수집
                logits = self.model.decoder(chunk_tensor, c)    # (1, N, 1)
...
def load_pretrained_weights(model, weight_path):
    print(f"사전 학습된 가중치 로드 시도: {weight_path}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        img_tensor = load_data_by_view_index(BASE_DIR, TARGET_VIEW_IDX, device)
    ...
    verts, faces, volume = generator.generate_mesh(img_tensor)
    
    if verts is not None:
        print("Restoring original scale and location...")
        
        # points.npz 파일에서 scale과 loc 정보 가져오기
    ...
    if verts is not None:
        ...
        o3d.visualization.draw_geometries([mesh], 
                                          window_name=f"Reconstructed Mesh(Baseline)", 
